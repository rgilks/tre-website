...
**Domain:** tre.systems
**Primary Goal:** A minimal, flashy portfolio that lists public GitHub projects using screenshots from `docs/screenshot.*`, descriptions from each repo, shows the latest activity first with a “Currently working on” highlight, and provides buttons for **Website** (if any) and **GitHub**.
...
* Project detail page also shows description, topics, and GitHub button.

---

## Architecture Overview

*(unchanged from previous spec except for PWA and project detail additions)*
...
* New project detail route with iframe overlay.
...
components/
  ProjectDetail.tsx        // Contains iframe and overlay logo link
...
* **Iframe**: fills viewport below header; `sandbox` attributes to allow scripts/forms unless restricted by target.
* **OverlayLogo**: fixed bottom-left, clickable, green glow; visible over iframe content.
* **Fallback**: if iframe cannot load, display screenshot, description, and a prominent “Open Project” button.
...
5. If homepageUrl exists and is embeddable, load in iframe; else show fallback.
6. Overlay TRE logo links back to `/`.
7. PWA passes Lighthouse checks: installable, offline home + cached thumbnails.
8. KV caching, cron refresh, images via CF Resizing.
9. Brand: black, terminal green, white accents.
10. Deployed at [https://tre.systems](https://tre.systems).
...
## 24) Project Viewer Route (Embedded)

**Route**: `/project/[owner]/[repo]`

**Behavior**

1. Server component resolves the project from the cached feed (`getProjects()`), finds `homepageUrl`.
2. **Embed check** (server‑side): attempt a `HEAD` to `homepageUrl` and inspect headers:
...
3. Render:
   ...
   * **Blocked** → display a branded fallback: large screenshot (if any), description, and two buttons: **Open Website** (new tab) and **GitHub**.
4. **TRE overlay logo**: fixed bottom‑left, circular 56px button (green on black, subtle glow). Clicking returns to Home (`/`).
...
## 25) Acceptance Criteria Additions (PWA + Viewer)
